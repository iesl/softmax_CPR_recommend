...
do
    ...
    if [[ "$datasets_mode" == "time" ]]; then
        ./run_single_model_log.sh $model $model_name $model_config $model_config_name ${dataset_arr[$i]} ${dataset_config_arr[$i]} ${dataset_name_arr[$i]} ${param_arr[$i]} #for time experiment
    else
