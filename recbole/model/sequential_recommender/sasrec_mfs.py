...
class SASRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.n_facet_hidden = config['n_facet_window'] #added for mfs
        self.softmax_nonlinear='None'
        self.efficient_mode = efficient_mode
        self.only_compute_loss = True
	self.n_embd = self.hidden_size
        ...
        for i in range(self.n_facet_all):
            ...
            linear_weights[:,:config.n_embd] = torch.eye(self.n_embd)
            #if i < n_facet:
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            if labels is not None:
                ...
                loss_raw = self.loss_fct(torch.log(prediction_prob.view(-1, self.n_items)+1e-8), labels)
                loss = loss_raw[labels != -100].mean()
            else:
